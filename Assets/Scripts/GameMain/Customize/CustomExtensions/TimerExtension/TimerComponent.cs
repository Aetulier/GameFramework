...
namespace GameName.Timer
{
    public class TimerComponent : CustomizeComponent, ICustomizeModule
    {
        ...
        /// <returns></returns>
        public async Task<bool> OnceTimerAsync(long time, UGFCancellationToken cancellationToken = null)
        {
            long nowTime = TimerTimeUtility.Now();
            if (time <= 0)
            {
        ...
        public async Task<bool> FrameAsync(UGFCancellationToken cancellationToken = null)
        {
