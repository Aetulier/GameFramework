...
namespace GameName.Editor.DataTableTools
{
    public sealed partial class DataTableProcessor
    {
        ...
        public DataTableProcessor(System.Data.DataTable Table, int nameRow, int typeRow, int? defaultValueRow, int? commentRow, int contentStartRow, int idColumn)
        {

            List<string[]> rawValues = new List<string[]>();
            int rawRowCount = Table.Rows.Count;
            int rawColumnCount = Table.Columns.Count;
            for (int i = 0; i < Table.Rows.Count; i++)
            {
                string[] rawValue = new string[Table.Columns.Count];
                for (int j = 0; j < Table.Columns.Count; j++)
                {
                    rawValue[j] = Table.Rows[i][j].ToString();
                }
