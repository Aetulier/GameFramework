...
namespace GameFramework.Fsm
{
    ...
    internal sealed class Fsm<T> : FsmBase, IReference, IFsm<T> where T : class
    {
        ...
        public static Fsm<T> Create(string name, T owner, object userData, params FsmState<T>[] states)
        {
            if (owner == null)
            {
        ...
        /// <param name="states">有限状态机状态集合。</param>
        /// <returns>创建的有限状态机。</returns>
        public static Fsm<T> Create(string name, T owner, object userData, List<FsmState<T>> states)
        {
            if (owner == null)
            {
                throw new GameFrameworkException("FSM owner is invalid.");
