...
namespace GameMain.Await
{
    public static partial class AwaitableExtensions
    {
        ...
        private static readonly Dictionary<int, TaskCompletionSource<REntity>> s_EntityTcs =
            new Dictionary<int, TaskCompletionSource<REntity>>();
        ...
        /// 打开界面（可等待）
        /// </summary>
        public static Task<UIForm> OpenUIFormAsync(this UIComponent uiComponent,string uiFormAssetName, string uiGroupName, object userData = null, int priority =0, bool pauseCoveredUIForm=false)
        {
#if UNITY_EDITOR
            TipsSubscribeEvent();
#endif
        ...
        /// 显示实体（可等待）
        /// </summary>
        public static Task<REntity> ShowEntityAsync<T>(this EntityComponent entityComponent, int entityId, string entityAssetName, string entityGroupName, int priority =0,object userData=null)
        {
#if UNITY_EDITOR
            TipsSubscribeEvent();
#endif
            var tcs = new TaskCompletionSource<REntity>();
            s_EntityTcs.Add(entityId, tcs);
            entityComponent.ShowEntity(entityId, typeof(T), entityAssetName, entityGroupName, priority, userData);
            return tcs.Task;
        }
